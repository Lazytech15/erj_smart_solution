...
const PLAN_CONFIG = {
  free_trial: {
    color: '#26c6da', colorDark: '#00acc1', colorLight: '#e0f7fa',
  ...
  starter: {
    color: '#26c6da', colorDark: '#0097a7', colorLight: '#e0f7fa',
  ...
  growth: {
    color: '#26a69a', colorDark: '#00897b', colorLight: '#e0f2f1',
  ...
  enterprise: {
    color: '#ef5350', colorDark: '#e53935', colorLight: '#ffebee',
...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: '#eef0f4', fontFamily: "'Inter', system-ui, sans-serif" }}>
      {/* Nav */}
      <header className="flex items-center justify-between px-8 py-4 bg-white" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <button onClick={() => navigate('/')} className="text-xs font-semibold text-slate-500 hover:text-slate-900 transition-colors flex items-center gap-1.5">
        ...
        <button onClick={() => navigate('/login')} className="text-xs font-semibold text-slate-500 hover:text-slate-900 transition-colors flex items-center gap-1.5">
      ...
      <main className="flex-1 flex flex-col items-center px-6 py-12">
        ...
        <div className="text-center mb-10 max-w-xl">
          <div className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-xs font-semibold mb-4" style={{ background: '#e8f5e9', color: '#2e7d32' }}>
          ...
          <p className="text-slate-500 text-sm">Pay per employee, scale as you grow. Cancel any time.</p>
        </div>
              ...
              >
                ...
                <div className="flex-1 px-5 py-4 space-y-4">
                  {FEATURE_GROUPS.map(group => {
                    const groupFeatures = group.features.filter(f => ALL_FEATURES.includes(f));
                    return (
                      <div key={group.label}>
                        <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 mb-2">{group.label}</p>
                            ...
                            return (
                              <li key={feature} className="flex items-center gap-2 text-xs">
                                <div
                                  ...
                                  style={{ width: 17, height: 17, borderRadius: '50%', background: included ? cfg.colorLight : '#f1f5f9' }}
                                >
                                  {included
        ...
        <div className="mt-14 max-w-2xl w-full">
          <p className="text-xs text-slate-400 font-semibold uppercase tracking-widest text-center mb-6">Common questions</p>
        ...
        <button onClick={() => navigate('/')} className="mt-10 text-xs font-semibold text-slate-400 hover:text-slate-700 transition-colors flex items-center gap-1.5">
          <ArrowLeft size={12} /> Back to homepage
        </button>
      </main>
    </div>
