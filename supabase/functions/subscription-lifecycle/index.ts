...
Deno.serve(async (req) => {
  ...
  const now = Date.now();
  const results = { grace: 0, suspended: 0, warned: 0, deleted: 0, errors: [] as string[] };

  const { data: subs, error } = await supabase
    .from("subscriptions")
    .select("subscription_id, status, billing, company, grace_started_at, final_warning_sent_at");
        ...
        if (adminEmail) {
          await sendEmail({
            ...
            body: `<p>We couldn't process your latest payment for <strong>${companyName}</strong>.</p>
                   <p>Your account is still fully active for now, but please update your billing details within
        ...
        if (adminEmail) {
          await sendEmail({
            ...
            body: `<p>Because payment hasn't been resolved, <strong>${companyName}</strong> has been suspended.</p>
                   ...
                   you reactivate. Please reactivate your plan to restore access.</p>`,
          });
        }
        results.suspended++;
        continue;
        ...
        if (adminEmail) {
          await sendEmail({
            ...
            body: `<p>This is a final notice. <strong>${companyName}</strong> has been suspended for 25 days.</p>
                   ...
                   <p>Reactivate your subscription now to prevent data loss.</p>`,
          });
        }
        results.warned++;
        continue;
